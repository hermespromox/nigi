...
export function buildMetrics({
  ...
  const areaVisitorsPerDay = roundUpToNearest(reviewVelocity * 1000, 500)
...
export function buildPublicSignals(analysis) {
  const metrics = analysis?.metrics || {}
  const score = Math.max(0, Math.min(100, Math.round(Number(analysis?.score) || 0)))
  const outlook = { strong: 'Excellent', promising: 'Promising', mixed: 'Selective', weak: 'Cautious' }
  const activePlaces = Number(metrics.activePoiCount || 0)
  const momentum = Number(metrics.activityIndex || 0)
  return {
    locationPotential: score,
    estimatedDailyFootfall: Math.max(0, Math.round(Number(metrics.areaVisitorsPerDay) || 0)),
    commercialOutlook: outlook[verdictForScore(score)],
    competitiveIntensity: activePlaces >= 25 ? 'Very high' : activePlaces >= 10 ? 'High' : activePlaces >= 4 ? 'Moderate' : 'Low',
    demandMomentum: momentum >= 25 ? 'Strong' : momentum >= 10 ? 'Active' : 'Emerging',
    customerReputation: Number(metrics.avgRating) > 0 ? Number(Number(metrics.avgRating).toFixed(2)) : null,
  }
}
...
export function validateExecutiveSummary(value) {
  const summary = cleanText(value, '', 1800)
...
export function validatePlacesStrategy(value, evidence) {
  ...
  return {
    ...
    reviewThemes,
    executiveSummary: validateExecutiveSummary(value?.executiveSummary),
    confidence,
  }
}
...
export function buildPlacesIntelligence(brief, evidence, rawStrategy) {
  ...
  const patternTemplates = {
    DENSE_COMPETITION: 'Competitive pressure is high: this area already supports a deep field of established operators.',
    CLOSE_COMPETITION: 'Direct competition is immediate around the candidate location.',
    STRONG_LEADERS: 'Established market leaders already command strong customer recognition and visibility.',
    HIGH_RATING_BAR: 'The strongest nearby operators set a high customer-rating benchmark.',
    UNEVEN_RATINGS: 'Customer sentiment is uneven across competitors, creating room for a more consistent experience.',
    REPEAT_BRAND_PRESENCE: 'Recurring brand presence signals competition from scaled operators as well as independents.',
    RECENT_MARKET_ACTIVITY: 'Current demand momentum is strong and commercially meaningful.',
    LOW_RECENT_ACTIVITY: 'Current demand momentum appears selective rather than broad-based.',
  }
  const opportunityTemplates = {
    ...
    STUDY_REVIEW_THEMES: 'Use the customer themes below to sharpen the offer before committing to the site.',
  ...
  return {
    headline: `Commercial outlook for this ${businessType}`,
    executiveSummary: strategy.executiveSummary,
    summary: `Nigi detects a ${Number(metrics.activePoiCount || 0) >= 25 ? 'high-pressure' : Number(metrics.activePoiCount || 0) >= 10 ? 'competitive' : 'developing'} local market with ${Number(metrics.activityIndex || 0) >= 25 ? 'strong' : Number(metrics.activityIndex || 0) >= 10 ? 'active' : 'emerging'} demand momentum. For a ${positioning} concept, the opportunity depends on a clear reason to choose you over the established alternatives.`,
    ...
    competitorHighlights: competitorIds.map((id) => byId.get(id)).filter(Boolean).map((place) => {
      ...
      const proximity = distance <= 250 ? 'an immediate' : distance <= 600 ? 'a nearby' : 'a local'
      const reputation = rating === null ? 'limited customer signal' : rating >= 4.6 ? 'excellent customer standing' : rating >= 4.2 ? 'strong customer standing' : 'mixed customer sentiment'
      const visibility = reviews >= 3000 ? 'entrenched market visibility' : reviews >= 500 ? 'established market visibility' : 'developing market visibility'
      return `${place.name} is ${proximity} competitor with ${reputation} and ${visibility}.`
    }),
    ...
    reviewThemes: strategy.reviewThemes.map((theme) => {
      const businesses = theme.placeIds.map((id) => byId.get(id)?.name).filter(Boolean)
      return { label: themeLabels[theme.code], businesses }
    }),
